 ...
 */
public class CoverageSumTest {

    @Test
    public void gridCoverageArraysTest() throws Exception{

        final ProcessDescriptor desc = ProcessFinder.getProcessDescriptor(GeotkProcessingRegistry.NAME, CoverageSumDescriptor.NAME);
        assertNotNull(desc);
        ...
        params.parameter(CoverageSumDescriptor.IN_FIRST_COVERAGE_NAME).setValue(cov1);
        params.parameter(CoverageSumDescriptor.IN_SECOND_COVERAGE_NAME).setValue(cov2);

        final Process process = desc.createProcess(params);
        final ParameterValueGroup result = process.call();

        GridCoverage resultGridCoverage = (GridCoverage) result.parameter(CoverageSumDescriptor.OUT_COVERAGE_NAME).getValue();
