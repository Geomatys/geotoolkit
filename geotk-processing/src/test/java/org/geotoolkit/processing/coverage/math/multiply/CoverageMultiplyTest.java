 ...
 */
public class CoverageMultiplyTest {

    @Test
    public void gridCoverageArraysTest() throws Exception{

        final ProcessDescriptor desc = ProcessFinder.getProcessDescriptor(GeotkProcessingRegistry.NAME, CoverageMultiplyDescriptor.NAME);
        assertNotNull(desc);
        ...
        params.parameter(CoverageMultiplyDescriptor.IN_FIRST_COVERAGE_NAME).setValue(cov1);
        params.parameter(CoverageMultiplyDescriptor.IN_SECOND_COVERAGE_NAME).setValue(cov2);

        final Process process = desc.createProcess(params);
        final ParameterValueGroup result = process.call();

        GridCoverage resultGridCoverage = (GridCoverage) result.parameter(CoverageMultiplyDescriptor.OUT_COVERAGE_NAME).getValue();
