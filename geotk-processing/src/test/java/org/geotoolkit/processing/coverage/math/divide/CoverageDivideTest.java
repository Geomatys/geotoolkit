 ...
 */
public class CoverageDivideTest {

    @Test
    public void gridCoverageArraysTest() throws Exception{

        final ProcessDescriptor desc = ProcessFinder.getProcessDescriptor(GeotkProcessingRegistry.NAME, CoverageDivideDescriptor.NAME);
        assertNotNull(desc);
        ...
        params.parameter(CoverageDivideDescriptor.IN_FIRST_COVERAGE_NAME).setValue(cov1);
        params.parameter(CoverageDivideDescriptor.IN_SECOND_COVERAGE_NAME).setValue(cov2);

        final Process process = desc.createProcess(params);
        final ParameterValueGroup result = process.call();

        GridCoverage resultGridCoverage = (GridCoverage) result.parameter(CoverageDivideDescriptor.OUT_COVERAGE_NAME).getValue();
