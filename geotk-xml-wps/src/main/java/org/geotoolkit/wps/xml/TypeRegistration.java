 ...
 * This class is not strictly necessary for marshalling a WPS object using JAXB, but makes
...
public abstract class TypeRegistration {
     ...
     * Those lists are determined dynamically from the modules found on the classpath.
     ...
     * <strong>That array shall not be modified.</strong> This method is currently for internal usage only,
     * because the {@code TypeRegistration} class is not part of public API. However if we add this functionality in a
     * future release (probably as an interface rather than exposing {@code TypeRegistration} itself), then we may
