...
class TransformingNamespaces implements NamespaceContext {
     ...
     */
    static NamespaceContext asXML(NamespaceContext context) {
        if (context != null) {
            if (context instanceof Inverse) {
                context = ((Inverse) context).context;
            } else {
                context = new TransformingNamespaces(context);
            }
        }
        return context;
    }
     ...
     */
    static NamespaceContext asJAXB(NamespaceContext context) {
        if (context != null) {
            if (context.getClass() == TransformingNamespaces.class) {
                context = ((TransformingNamespaces) context).context;
            } else {
                context = new TransformingNamespaces.Inverse(context);
            }
        }
        return context;
    }
     ...
     */
    private TransformingNamespaces(final NamespaceContext context) {
    ...
    private static final class Inverse extends TransformingNamespaces {
        ...
        Inverse(final NamespaceContext context) {
            super(context);
        }
        ...
        switch (prefix) {
            case "ows": return OWS_2_0_NAMESPACE;
            case "wps": return WPS_2_0_NAMESPACE;
        }
    ...
    public String getNamespaceURI(final String prefix) {
        switch (prefix) {
            case "ows": return OWS_1_1_NAMESPACE;
            case "wps": return WPS_1_0_NAMESPACE;
        }
    ...
    @Override
    public String getPrefix(final String namespaceURI) {
        final String kp = getKnownPrefix(namespaceURI);
        return kp == null? context.getPrefix(namespaceURI) : kp;
    }
    ...
    public Iterator<String> getPrefixes(final String namespaceURI) {
        final String kp = getKnownPrefix(namespaceURI);
        return kp == null?
                context.getPrefixes(namespaceURI) : Collections.singletonList(kp).iterator();
    }

    private static String getKnownPrefix(final String namespaceURI) {
        switch (namespaceURI) {
