...
public final class DggalArrayZone implements AutoCloseable {
    ...
    public void set(long[] zones) throws Throwable {
        if (zones.length != getCount()) {
            throw new IllegalArgumentException("Was expecting a table of length " + getCount());
        }

        final MemorySegment mem = (MemorySegment) dggal.DGGAL_Array_DGGRSZone_getPointer.invokeExact(pointer);
        final LongBuffer buffer = mem.reinterpret(zones.length*8).asByteBuffer().order(ByteOrder.LITTLE_ENDIAN).asLongBuffer();
        buffer.put(zones);
    }
