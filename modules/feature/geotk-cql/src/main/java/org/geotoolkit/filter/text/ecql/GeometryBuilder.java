...
abstract class GeometryBuilder {
    ...
    protected Coordinate[] asCoordinate(final Stack<Coordinate> stack) {
    ...
    protected List<Geometry> popGeometry(final int geometryNode) throws CQLException{
        final List<Geometry> geomList = new LinkedList<Geometry>();
        while(!getResultStack().empty()) {
            final Result result = getResultStack().peek();
            ...
            final Geometry geometry = (Geometry)result.getBuilt();
    ...
    protected List<Geometry> popGeometryLiteral(final int geometryNode) throws CQLException{
        ...
        while(!getResultStack().empty()) {
            ...
            final Literal geometry = (Literal)result.getBuilt();
