...
class CategoryList extends AbstractList<Category> implements MathTransform1D, Comparator<Category>, Serializable {
    ...
    /**
     * The inverse transform, never {@code null}. The following rule must hold:
     * <p>
     * <ul>
        ...
        for (int i=0; i<categories.length; i++) {
            ...
            if (i != 0) {
                ...
                if (compare(minimum, previous.maximum) <= 0) {
                    ...
                    throw new IllegalArgumentException(Errors.format(Errors.Keys.RangeOverlap_4, args));
                }
     ...
     * {@linkplain org.geotoolkit.coverage.grid.ViewType#GEOPHYSICS geophysics} and {@code false} is
     * for {@linkplain org.geotoolkit.coverage.grid.ViewType#PACKED packed} data. This method is used
    ...
    private static void checkDimension(final DirectPosition point) {
        ...
        if (dim != 1) {
            throw new MismatchedDimensionException(Errors.format(
                    Errors.Keys.MismatchedDimension_2, 1, dim));
        }
    }
    ...
    public final double derivative(final double value) throws TransformException {
             ...
             doubleToRawLongBits(value) != doubleToRawLongBits(category.minimum))
        {
            category = getCategory(value);
            if (category == null) {
                throw new TransformException(Errors.format(Errors.Keys.NoCategoryForValue_1, value));
            }
            last = category;
        }
                           ...
                           int numPts) throws TransformException
    {
        ...
        for (int peekOff=srcOff; true; peekOff += direction) {
            ...
            final MathTransform1D step = category.transform;
            if (srcFloat != null) {
                if (dstFloat != null) {
            ...
            if (overflowFallback != null) {
                ...
                final Category inverse = category.inverse;
    ...
    public final void transform(final WritableRectIter iterator) throws RasterFormatException {
        ...
        try {
            ...
            RasterFormatException exception = new RasterFormatException(Errors.format(
                    Errors.Keys.IllegalTransformForType_1, Classes.getClass(tr)));
