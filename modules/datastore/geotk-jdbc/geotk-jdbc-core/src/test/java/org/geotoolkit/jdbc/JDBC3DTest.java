...
/**
 * Tests the ability of the featurestore to cope with 3D data
 *
...
public abstract class JDBC3DTest extends JDBCTestSupport {
    ...
    protected void setUp() throws Exception {
        ...
        line3DType = FeatureTypeUtilities.createType(featureStore.getNamespaceURI() + "." + tname(LINE3D),
                aname(ID) + ":0," + aname(GEOM) + ":LineString:srid=4359," + aname(NAME)
                        + ":String");
        poly3DType = FeatureTypeUtilities.createType(featureStore.getNamespaceURI() + "." + tname(POLY3D),
    ...
    public void testSchema() throws Exception {
        SimpleFeatureType schema = (SimpleFeatureType) featureStore.getFeatureType(tname(LINE3D));
    ...
    public void testReadPoint() throws Exception {
        FeatureCollection fc = featureStore.createSession(false).getFeatureCollection(QueryBuilder.all(nsname(POINT3D)));
    ...
    public void testReadLine() throws Exception {
        FeatureCollection fc = featureStore.createSession(false).getFeatureCollection(QueryBuilder.all(nsname(LINE3D)));
        FeatureIterator<SimpleFeature> fr = fc.iterator();
        assertTrue(fr.hasNext());
    ...
    public void testWriteLine() throws Exception {
        ...
        List<FeatureId> fids = FeatureStoreUtilities.write(featureStore.getFeatureWriterAppend(nsname(LINE3D)), col);


        // retrieve it back
        FeatureIterator<SimpleFeature> fi = featureStore.getFeatureReader(
     ...
     * featurestore and retrieves it back to make sure 3d data is really handled as
     * such
     *
     * @throws Exception
     */
    public void testCreateSchemaAndInsert() throws Exception {
        ...
        // insert it
        FeatureWriter<SimpleFeatureType, SimpleFeature> fw = featureStore.getFeatureWriterAppend(
                nsname(POLY3D));
        ...
        FeatureReader<SimpleFeatureType, SimpleFeature> fr = featureStore.getFeatureReader(
                QueryBuilder.all(featureStore.getFeatureType(tname(POLY3D)).getName()));
        assertTrue(fr.hasNext());
        f = fr.next();
    ...
    public void testBounds() throws Exception {
        JTSEnvelope2D env = (JTSEnvelope2D) featureStore.getEnvelope(QueryBuilder.all(nsname(LINE3D)));
