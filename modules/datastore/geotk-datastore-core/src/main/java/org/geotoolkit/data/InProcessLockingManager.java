...
public class InProcessLockingManager implements LockingManager {
    ...
    public synchronized void lockFeatureID(final String typeName, final String featureID,
        ...
        while (lock != null) {
            ...
            if (lock instanceof TransactionLock) {
                final TransactionLock tlock = (TransactionLock) lock;
                ...
                final MemoryLock mlock = (MemoryLock) lock;
     ...
     */
    protected Lock getLock(final String typeName, final String featureID) {
     ...
     */
    protected Map locks(final String typeName) {
     ...
     *
     * @see LockingManager#exists(java.lang.String)
     */
    @Override
     ...
     */
    public boolean isLocked(final String typeName, final String featureID) {
    ...
    class MemoryLock implements Lock {
        ...
        MemoryLock(final FeatureLock lock) {
            this(lock.getAuthorization(), lock.getDuration());
        }

        MemoryLock(final String id, final long length) {
            authID = id;
        ...
        @Override
        public boolean isAuthorized(final Transaction transaction) {
            //LOGGER.info("checking authorization on " + this.toString() + ", "
            //  + ((transaction != Transaction.AUTO_COMMIT)
            //  ? transaction.getAuthorizations().toString() : "autocommit"));

            return (transaction != Transaction.AUTO_COMMIT) && transaction.getAuthorizations().contains(authID);
        }
