...
public class BatchFeatureEvent extends FeatureEvent {
     ...
     */
    public void add(final FeatureEvent change) {
        if (change.getType() == Type.ADDED) {
            if (change.getFilter() instanceof Id) {
                // store these feature Ids for later
                final Id newFeatureIds = (Id) change.getFilter();
     ...
     */
    public void replaceFid(final String tempFid, final String actualFid) {
        for (Identifier id : fids) {
            if (tempFid.equals(id.getID())) {
                // we have a match!
                if (id instanceof DefaultFeatureId) {
