/*
 *    GeoTools - The Open Source Java GIS Toolkit
 *    http://geotools.org
 *
 *    (C) 2004-2008, Open Source Geospatial Foundation (OSGeo)
 *
 *    This library is free software; you can redistribute it and/or
 *    modify it under the terms of the GNU Lesser General Public
 *    License as published by the Free Software Foundation;
 *    version 2.1 of the License.
 *
 *    This library is distributed in the hope that it will be useful,
 *    but WITHOUT ANY WARRANTY; without even the implied warranty of
 *    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
 *    Lesser General Public License for more details.
 */

package org.geotools.report.simple.string;

import java.awt.Image;
import org.geotools.report.JRMapperFactory;
import org.geotoolkit.util.SimpleInternationalString;
import org.opengis.util.InternationalString;

/**
 * Static String mapper factory. create static string mappers that can be used
 * against any kind of records.
 *
 * @author Johann Sorel (Geomatys)
 */
public class StaticStringMapperFactory implements JRMapperFactory<String,Object>{

    private static final InternationalString TITLE = new SimpleInternationalString("Static text");

    /**
     * {@inheritDoc }
     */
    @Override
    public Class<String> getValueClass() {
        return String.class;
    }

    /**
     * {@inheritDoc }
     */
    @Override
    public StaticStringMapper createMapper() {
        return new StaticStringMapper(this);
    }

    public StaticStringMapper createMapper(String value) {
        return new StaticStringMapper(value,this);
    }

    /**
     * {@inheritDoc }
     */
    @Override
    public Image getIcon(int type) {
        return null;
    }

    /**
     * {@inheritDoc }
     */
    @Override
    public InternationalString getTitle() {
        return TITLE;
    }

    /**
     * {@inheritDoc }
     */
    @Override
    public Class getFieldClass() {
        return String.class;
    }

    /**
     * {@inheritDoc }
     */
    @Override
    public Class<Object> getRecordClass() {
        return Object.class;
    }

    /**
     * {@inheritDoc }
     */
    @Override
    public String[] getFavoritesFieldName() {
        return new String[0];
    }

}
