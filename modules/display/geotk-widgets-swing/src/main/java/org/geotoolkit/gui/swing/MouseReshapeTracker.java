 ...
 * @module
 *
 * @todo Consider renaming get/setClip into something else.
 * @todo Be more consistent on "visor", "rectangle" and "rectangular shape" wording.
 *       The later don't said enough what there are about.
 */
class MouseReshapeTracker extends MouseInputAdapter implements Shape {
    ...
    /**
     * Constructs an object capable of moving and resizing a rectangular shape through mouse
     * movements. The given shape does not have to be a rectangle. It could, for example, be
     * a circle. The coordinates of this shape will be the initial coordinates of the visor.
     * They are logical coordinates and not pixel coordinates.
     * <p>
     * Note that the constructor retains a direct reference to this shape, without creating a
     * clone. As a consequence, any modification carried out on the geometric shape will have
     * repercussions for this {@code MouseReshapeTracker} object and vice versa.
     *
     * @param shape Rectangular geometric shape to be resized by mouve movements.
     */
     ...
     *
     * @param newTransform The transform from logical to pixel coordinates.
     */
     ...
     * @param max The new maximam value.
     *
     * @see #getMinX
     * @see #getMaxX
     * @see #getCenterX
     ...
     * should be expressed in logical coordinates. This method can conservatively return
    ...
    /**
     * Indicates whether the rectangular shape contains the specified rectangle. This rectangle
     * must be expressed in logical coordinates. This method can conservatively return
     * {@code false} as permitted by the {@link Shape} specification.
     *
     ...
     * must be expressed in logical coordinates. This method can conservatively return {@code true}
     * as permitted by the {@link Shape} specification.
     *
     ...
     * {@link #setClipMinMax setClipMinMax(...)} with the appropriate parameters. It is defined
     * in order to avoid confusion amongst programmers used to <em>Java2D</em> conventions. If you
     * want to specify infinite values (in order to widen the visor's bounds to all possible values
     * along certain axes), you <u>must</u> use {@link #setClipMinMax setClipMinMax(...)} rather than
     * {@code setClip(...)}.
     *
     ...
     * The value {@link java.lang.Double#NaN} for a given argument indicates that we want to keep
     * the old value.
     * <p>
     * If the visor doesn't fit completely within the new bounds, it will be moved and resized as
     * needed in order to make it fit.
     */
    ...
    /**
     * Method called automatically when a change in the clip is required. This method can be called,
     ...
     * does nothing, which means that no automatic change in the clip will be authorised.
     */
    protected void clipChangeRequested(double xmin, double xmax, double ymin, double ymax) {
    }
    ...
    /**
     * Specifies whether the rectangle can be moved with the mouse. The value {@code false}
     * indicates that the rectangle cannot be moved, but can still be resized if
     * {@link #setAdjustable} has been called with the appropriate parameters.
     */
    ...
    /**
     * Indicates whether the size of a rectangle can be modified using a specified edge. The
     * specified edge must be one of the following constants:
     * <p>
     * <table border align=center cellpadding=8 bgcolor=floralwhite><tr>
     ...
     * </tr></table>
     * <p>
     * These constants designate the edge which is visible on screen. For example, {@code NORTH}
     * always designates the top edge on the screen. However, this could correspond to another
     * edge of the logical shape {@code this} depending on the affine transform which was
     * specified during the last call to {@link #setTransform}. For example,
     * {@code AffineTransform.getScaleInstance(+1,-1)} has the effect of inverting the <var>y</var>
     * axis so that the <var>y</var><sub>max</sub> values appear to the North rather than the
    ...
    /**
     * Method called automatically when the user presses a mouse button anywhere within the
     * component. The default implementation checks if the button was pressed whilst the mouse
     * cursor was within the rectangle.  If so, this object will track the mouse drags to move
     * or resize the rectangle.
     */
    @Override
    ...
    /**
     * Method called automatically during mouse drags. The default implementation applies the
     * mouse movement to the rectangle and notifies the component where the event which it needs
     * to redraw, at least in part, came from.
     */
    @Override
    public void mouseDragged(final MouseEvent e) {
        if (isDragging) {
            ...
            if (source != null) try {
                 ...
                 * (mouseDX, mouseDY) coordinates represent the position of the mouse at the
                 * moment the button is pressed and don't normally change (except during certain
                 * adjustments).  In determining (mouseDX, mouseDY), they is calculated as if the
                 * user began to drag the rectangle at the very corner, though in reality they
                 * could have clicked anywhere.
                 */
                ...
                /*
                 * Modifies the rectangle's coordinates and signals that the component needs redrawing.
                 * Note: 'repaint' should be called before and after 'setFrame' because the coordinates
                 * changed.
                 */
    ...
    /**
     * Method called automatically when the user releases the mouse button. The default
    ...
    /**
     * Method called automatically <strong>before</strong> the position or the size of the
     * visor has changed. A call to {@code stateWillChange} is normally followed by a call to
     * {@link #stateChanged}, <u>except</u> if the expected change didn't ultimately occur.
     * The derived classes can redefine this method to take the necessary actions when a change
     * is on the point of being actioned. They must not, however, call any method which risks
     * modifying the state of this object.  The default implementation does nothing.
     *
    ...
    /**
     * Method called automatically <strong>after</strong> the position and size of the visor
     * has changed. The call to {@code stateChanged} must have been preceded by a call to
     * {@link #stateWillChange}. The derived classes can redefine this method to take the
     * necessary actions when a change has just been actioned.  They must not, however, call
     * any method which risks modifying the state of this object. The default implementation
     * does nothing.
     *
     * @param isAdjusting {@code true} if the user is still modifying the position of the visor,
     *        {@code false} if they have released the mouse button.
    ...
    /**
     * Method called automatically before the position or the size of the visor has changed.
     */
    ...
    /**
     * Method called automatically after the position or the size of the visor has changed.
     */
    ...
    /**
     * Updates the text in the editors. Each editor added by the method {@link #addEditor} will
     * have its text reformatted.  This method can be called, for example, after changing the
     * format used by the editors. It is not necessary to call this method each time the mouse
     * moves; it is done automatically.
     */
    ...
    /**
     * Adds an editor in which the user can explicitly specify the coordinates of one of the edges
     * of the rectangle. Each time the user drags the rectangle, the text appearing in this editor
     * will automatically be updated. If the user explicitly enters a new value in this editor, the
     * position of the rectangle will be adjusted.
     *
     ...
     * </tr></table>
     *
     * @param  toRepaint Component to repaint after a field has been edited, or {@code null}
     *         if there isn't one.
     * @return An editor in which the user can specify the position of one of the edges of
     *         the geometric shape.
     * @throws IllegalArgumentException if {@code side} isn't one of the recognised codes.
     */
    ...
    /**
     * Removes an editor from the list of those which display the coordinates of the visor.
     *
    ...
    private final class Control implements PropertyChangeListener {
        ...
        /**
         * Side of the rectangle to be controlled. This field designates the edge which is visible on
         * screen. For example, {@code NORTH} always designates the top edge on the screen. However,
         * this could correspond to another edge of the logical shape {@link MouseReshapeTracker}
         * depending on the affine transform that was specified during the last call to
         * {@link MouseReshapeTracker#setTransform}. For example,
         * {@code AffineTransform.getScaleInstance(+1,-1)} has the effect of inverting the
         * <var>y</var> axis so that the <var>y</var><sub>max</sub> values appear to the North
         ...
         * @param isDate {@code true} if the field {@link #editor} formats dates, or {@code false}
         *        if it formats numbers.
         * @param side Edge of the rectangle to control. This argument designates the edge visible
         *        on screen.
         * @param toRepaint Component to repaint after the field has been edited,
        ...
        /**
         * Method called automatically each time the value in the editor changes.
         */
        @Override
        ...
        /**
         * Called each time the position of the rectangle is adjusted. This method will adjust
         * the value displayed in the text field based on the position of the rectangle.
         */
