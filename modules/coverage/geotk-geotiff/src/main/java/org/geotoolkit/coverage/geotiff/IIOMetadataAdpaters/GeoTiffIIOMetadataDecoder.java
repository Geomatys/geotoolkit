...
public final class GeoTiffIIOMetadataDecoder {
    ...
    public String getGeoKey(final int keyID) {
        ...
        if (rec.getTiffTagLocation() == 0) // value is stored directly in the GeoKey record
        {
            return String.valueOf(rec.getValueOffset());        // value is stored externally
    ...
    public GeoKeyEntry getGeoKeyRecordByIndex(int index) {
        index *= 4;
        return new GeoKeyEntry(getIntValueAttribute(geoKeyDirEntries.item(index)), getIntValueAttribute(geoKeyDirEntries.item(index + 1)), getIntValueAttribute(geoKeyDirEntries.item(index + 2)), getIntValueAttribute(geoKeyDirEntries.item(index + 3)));
    ...
    private IIOMetadataNode getTiffField(final int tag) {
        if (tag == GeoTIFFTagSet.TAG_GEO_ASCII_PARAMS && this.geoKeyAsciiParams != null) {
            return this.geoKeyAsciiParams;
        }
        if (tag == GeoTIFFTagSet.TAG_GEO_DOUBLE_PARAMS && this.geoKeyDoubleParams != null) {
            return this.geoKeyDoubleParams;
        }
        if (tag == GeoTIFFTagSet.TAG_GEO_KEY_DIRECTORY && this.geoKeyDir != null) {
            return this.geoKeyDir;        // embed the exit condition in the for loop
        }
        Node child = null;
        Node number = null;
    ...
    private int getTiffShort(final IIOMetadataNode tiffField, final int index) {
        ...
        return getIntValueAttribute(((IIOMetadataNode) tiffField.getFirstChild()).getElementsByTagName(
    ...
    private String getTiffAscii(final IIOMetadataNode tiffField,
        ...
        return getValueAttribute(
                ((IIOMetadataNode) tiffField.getFirstChild()).getElementsByTagName(
                GeoTiffConstants.GEOTIFF_ASCII_TAG).item(0)).substring(start, start + length - 1);
