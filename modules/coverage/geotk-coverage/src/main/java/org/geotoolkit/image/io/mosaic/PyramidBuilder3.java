...
import org.geotoolkit.image.io.plugin.WorldFileImageWriter.Spi;
...
public class PyramidBuilder3 {
     ...
     * @see Resample
     */
    public PyramidBuilder3(final RenderedImage renderedImage, final double[] coeffX, final double[] coeffY,
...
//                    TiledImage tuile = new TiledImage(tempMinx, miny, tmaxx - tempMinx, tmaxy - miny, tempMinx, miny, sampleMod, colorMod);
                    TiledImage tuile = new TiledImage(tempMinx, miny, tmaxx - tempMinx, tmaxy - miny, tileGridXOffset, tileGridYOffset, sampleMod, colorMod);
                    //on rempli la tuile
...
//                        destPix.next();
                    ...
                    //on genere un nom
                    String pathTile = tileDirectory.getPath() + "/"+formatter.generateFilename(lev, itx, ity)+".";
                    namePTile = pathTile+formatName;
                    String namePTile2 = pathTile+"TFW";
                    //on genere le path
...
//                    ImageWriter writer = XImageIO.getWriterByFormatName("TIFF", new File(namePTile2), tuile);
                    ImageWriter writer = XImageIO.getWriterByFormatName("tiff-wf", null, null);
                    writer.setOutput(new File(namePTile));
                    writer.write(tuile);
                    writer.dispose();
//                    assert(writer instanceof WorldFileImageWriter) : "le writer n'est pas de type worldFile";
//                    writer.write(tuile);
//                    writer.dispose();
