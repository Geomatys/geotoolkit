 ...
 */
public class RowMajorWritableIteratorTest extends RowMajorIteratorTest{
    ...
    public void rectUpperLeftWriteTest() {
        ...
        fillGoodTabRef(minx, miny, width, height, numBand, rect);
        setPixelIterator(renderedImage);
        int comp = 0;
        while (pixIterator.next()) tabTest[comp++] = pixIterator.getSample();
        assertTrue(compareTab(tabTest, tabRef));
     ...
     */
    private void fillGoodTabRef(int minx, int miny, int width, int height, int numBand, Rectangle areaIterate) {
