...
public class DefaultFloatIteratorTest extends DefaultReadTest {
    ...
    protected void setRasterTest(int minx, int miny, int width, int height, int numBand, Rectangle subArea) {
        assert (width*height*numBand/2 <= 32000) : "image too large to float testing";
        int comp = 0;
        ...
        for (int y = miny; y<miny + height; y++) {
            for (int x = minx; x<minx + width; x++) {
                for (int b = 0; b<numBand; b++) {
                    rasterTest.setSample(x, y, b, comp++-32000);
                }
            }
        }
        int mx, my, w,h;
        ...
        for (int y = my; y<my + h; y++) {
            for (int x = mx; x<mx + w; x++) {
                for (int b = 0; b<numBand; b++) {
                    tabRef[comp++] = (float) (b + ((x-minx) + (y-miny) * width) * numBand - 32000);
                }
            }
        }
    }
            ...
            for (int tileX = tileMinX; tileX<tileMaxX; tileX++) {
                ...
                for (int y = depY; y<endY; y++) {
                    for (int x = depX; x<endX; x++) {
                        for (int b = 0; b<numBand; b++) {
                            tabRef[comp++] =  (float) (b + ((x-depX) + (y-depY) * tilesWidth)*numBand - 32000);
                        }
                    }
                }
            }
