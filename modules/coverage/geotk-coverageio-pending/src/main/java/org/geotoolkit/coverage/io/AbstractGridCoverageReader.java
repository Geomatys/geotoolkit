...
public abstract class AbstractGridCoverageReader {
            ...
            throws IOException
    {
        ...
        if (input != null) {
            ...
            throw new IllegalArgumentException(getString(Errors.Keys.NO_IMAGE_READER));
        }
    }
    ...
    public synchronized int getNumImages(final boolean allowSearch) throws IOException {
        if (reader == null) {
            throw new IllegalStateException(getString(Errors.Keys.NO_IMAGE_INPUT));
        }
    ...
    final int checkImageIndex(final int imageIndex) throws IOException, IndexOutOfBoundsException {
        ...
        if (imageIndex<reader.getMinIndex() || (imageIndex>=numImages && numImages>=0)) {
            throw new IndexOutOfBoundsException(Errors.getResources(locale).getString(
                    Errors.Keys.ILLEGAL_ARGUMENT_$2, "imageIndex", imageIndex));
        }
        return numImages;
    }
    ...
    public synchronized String getName(final int index) throws IOException {
        ...
        if (file != null) {
            ...
            name = Vocabulary.getResources(locale).getString(Vocabulary.Keys.UNTITLED);
        }
        if (numImages != 1) {
            name = name + " [" + index + ']';
        }
     ...
     * envelope.
     *
     * The default implementation construct a {@link GridEnvelope} object
