...
public class RawBinaryImageReadParam extends ImageReadParam {
    ...
    /**
     * The target data type, or {@link DataBuffer#TYPE_UNDEFINED} if not defined.
     * In the later case, the target data type will be the same than the raw one.
     */
    ...
    /**
     * Returns the image size in the input stream, or {@code null} if unknow. Image size is
     * specified by the last call to {@link #setStreamImageSize} or {@link #setStreamSampleModel}.
     *
     * @return The image size in the input stream.
     */
     ...
     * {@link DataBuffer#TYPE_UNDEFINED} reset the default value, which is reader dependent.
     *
     * @param dataType The data type, or {@link DataBuffer#TYPE_UNDEFINED} if unknow.
     *        Know data type should be a constant from {@link DataBuffer}. Common types
     *        are {@link DataBuffer#TYPE_INT}, {@link DataBuffer#TYPE_FLOAT} and
    ...
    /**
     * Returns the data type in input stream, or {@link DataBuffer#TYPE_UNDEFINED} if unknow.
     * Data type is specified by the last call to {@link #setStreamDataType} or
     * {@link #setStreamSampleModel}.
     *
     * @return The data type in the input stream.
     */
    ...
    /**
     * Sets the desired image type for the destination image, using one of {@link DataBuffer}
     * enumeration constant. This setting will override any previous setting made with
     * {@link #setDestinationType(ImageTypeSpecifier)} or this {@code setDestinationType(int)}
     * method.
     *
    ...
    final ImageTypeSpecifier getDestinationType(final int numBands) {
        ...
        if (model!=null) {
            /*
             * Case 1: we know the sample model for data in the underlying stream. We will
             *         use the same model for the memory image, just changing the data type.
             */
            ...
            /*
             * Case 2: We have to create a sample model from scratch. We will use a banded
             *         sample model with some arbitrary color space (which may be changed
             *         after the image reading is completed).
             */
        ...
        if (sampleModel instanceof ComponentSampleModel) {
            ...
            colorModel = new ComponentColorModel(getColorSpace(numBands),
                    false, false, Transparency.OPAQUE, sampleModel.getDataType());
        } else {
    ...
    /**
     * Sets a sample model indicating the data layout in the input stream. Indications comprise
     * image size and data type, i.e. calling this method with a non-null value is equivalent to
     * calling also the following methods:
     *
     * {@preformat java
     ...
     * }
     *
     * Setting the sample model to {@code null} reset the default model, which is reader dependent.
     *
     * @param model The sample model to use for the image to be read.
     */
    ...
    /**
     * Returns a sample model indicating the data layout in the input stream. The
     * {@link SampleModel}'s width and height should matches the image size in the
     * input stream.
     *
     ...
     * input stream.
     *
     * @param  defaultSampleModel A default sample model, or {@code null} if there is no default.
     *         If this {@code RawBinaryImageReadParam} contains unspecified sample model, image
     *         size or data type, values from {@code defaultSampleModel} will be used.
     * @return A sample model indicating the data layout in the input stream, or {@code null}
     *         if unknow.
     */
    ...
    private static SampleModel getStreamSampleModel(final SampleModel defaultSampleModel,
            SampleModel model, Dimension size, int dataType)
    {
