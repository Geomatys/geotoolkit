...
public class ImageGeometry extends MetadataAccessor {
    ...
    /**
     * Returns the number of dimensions. If the {@link #low} array and the cells don't have
     * the same dimension, then a warning is logged and the smallest dimension is returned.
     * If one of them is empty, the dimension of the oter one is then returned.
     *
     * @return The number of dimension in the grid.
     */
     ...
     * @return The range of grid index along the specified dimension.
     */
    ...
    public void setGridRange(final int dimension, final int minimum, final int maximum) {
        ...
        if (lows == null) {
            ...
            if (length > oldLength) {
                lows = Arrays.copyOf(lows, length);
            }
        }
        if (highs == null) {
            ...
            if (length > oldLength) {
                highs = Arrays.copyOf(highs, length);
            }
        }
     ...
     *
     * @param  dimension The dimension index, from 0 inclusive to {@link #getDimension} exclusive.
     * @return The range of ordinate value along the specified dimension.
     */
    public NumberRange<Double> getOrdinateRange(final int dimension) {
        ...
        return NumberRange.create(lower, upper);
    }

    /**
     * Sets the envelope range along the specified dimension. If the dimension is greater
     * than the current envelope dimension, then this dimension is added.
     *
     * @param dimension The dimension to set. It can eventually be greater than {@link #getDimension}.
     * @param minimum   The minimum value along the specified dimension (inclusive).
    ...
    public void setOrdinateRange(final int dimension, final double minimum, final double maximum) {
        ...
        if (lowers == null) {
            ...
            if (length > oldLength) {
                lowers = Arrays.copyOf(lowers, length);
        ...
        if (uppers == null) {
            ...
            if (length > oldLength) {
                uppers = Arrays.copyOf(uppers, length);
     ...
     *
     * @param dimension The dimension index, from 0 inclusive to {@link #getDimension} exclusive.
     * @return The ordinate values along the specified dimension, or {@code null}.
     */
    ...
    /**
     * Sets the ordinate values along the specified dimension. The minimum and
     ...
     * will increase the envelope {@linkplain #getDimension dimension} by one. This
     * method can be invoked in replacement of {@link #addOffsetVector} when every
     * cell coordinates need to be specified explicitly.
     *
     ...
     * {@linkplain PixelOrientation#UPPER_LEFT upper left} corner. In some OGC specifications,
     * this is often the pixel {@linkplain PixelOrientation#CENTER center}.
     *
     * @return The pixel orientation (usually {@code "center"}, {@code "lower left"},
     *         {@code "lower right"}, {@code "upper right"} or {@code "upper left"}),
    ...
    /**
     * Sets the pixel orientation to the specified value. The pixel orientation gives
