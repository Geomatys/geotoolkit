...
public final class ChannelImageInputStreamTest {
    /**
     * Fill a buffer with random data and compare the result with a standard image input stream.
    ...
    public void testWithRandomData() throws IOException {
        ...
        int operation = 0;
        try {
            while ((position = r.getStreamPosition()) < buffer.length - 256) {
                ...
                switch (operation) {
                    ...
                    case 12: {
                        final int n = random.nextInt(65);
                        assertEquals("readBits(" + n + ')', r.readBits(n), t.readBits(n));
                        break;
                    }
                    case 13: {
                        final int length = random.nextInt(256);
                    ...
                    case 15: {
                        final int length = random.nextInt(128);
                    ...
                    case 18: {
                        final int length = random.nextInt(32);
