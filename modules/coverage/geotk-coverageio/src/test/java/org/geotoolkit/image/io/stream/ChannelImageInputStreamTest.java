...
public final strictfp class ChannelImageInputStreamTest {
    ...
    public void testWithRandomData() throws IOException {
        ...
        try (ImageInputStream r = ImageIO.createImageInputStream(new ByteArrayInputStream(buffer));
             ImageInputStream t = new ChannelImageInputStream(Channels.newChannel(new ByteArrayInputStream(buffer)), 128))
        {
            long position = 0;
            int bitOffset = 0;
            int operation = 0;
    ...
    public void testUsingFile() throws IOException {
        ...
        try (FileInputStream in = new FileInputStream(file);
             ImageInputStream r = ImageIO.createImageInputStream(new FileInputStream(file));
             ImageInputStream t = new ChannelImageInputStream(in.getChannel(), 128))
        {
            final long          seed = System.nanoTime();
            final Random      random = new Random(seed);
