...
/**
 * Creates image transformation grid for the given {@link MathTransform2D}.
 ...
 * @version 3.21
 *
 * @since 3.21 (derived from 3.16)
 * @module
 */
final class TransformGrid {
    /**
     * The minimal size, in pixels. If the cell size is lower than this threshold,
     * we will abandon the attempt to create a grid.
     */
    ...
    /**
     * The (x,y) coordinate of the upper-left corner in the source CRS.
     * (Reminder: "source" here is relative to the math transform.
     * This is usually the pixel coordinates of the target image).
     */
    final int xmin, ymin;
     ...
     * Number of cells.
     */
    final int xNumCells, yNumCells;

    /**
     * Sequence of (x,y) grid coordinates, in row-major fashion.
     * This is {@code null} if only {@link #globalTransform} shall be used.
     */
    final double[] warpPositions;

    /**
     * An affine transform that can be used on the domain as a whole. This affine transform maps
     * pixel corner (not pixel center). User shall translate the transform for mapping pixel centers
     * before to use is in a raster resample operation.
     *
     * <p>In current implementation, this is {@code null} if {@link #warpPositions} is non-null.
     * However a future implementation may provide a non-null value in every cases.</p>
     */
    final AffineTransform globalTransform;

    /**
     * Creates a new instance for the given affine transform.
     */
    private TransformGrid(final AffineTransform tr) {
        xmin      = 0;
        ymin      = 0;
        xStep     = 0;
        yStep     = 0;
     ...
     */
    private TransformGrid(final int xmin, final int xStep, final int xNumCells,
    ...
    /**
     * Creates a grid for the given transform. This method may round the affine transform coefficients,
     * because integer scale factors can make the rendering much faster by allowing Java2D to use optimized
     * code path (for example using integer arithmetic).
     * <p>
