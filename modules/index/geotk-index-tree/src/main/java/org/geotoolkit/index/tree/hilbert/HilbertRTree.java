...
public class HilbertRTree extends DefaultAbstractTree {
    ...
    public static void insertNode(final DefaultNode candidate, final GeneralEnvelope entry) {
        ...
        if (candidate.isFull()) {
            ...
            if (lSp != null) {
                ...
                ((List<DefaultNode>) candidate.getUserProperty("cells")).clear();
        ...
        if (candidate.isLeaf()) {
            ...
            ((HilbertNode) candidate).setBound(null);
        }
    }
    ...
    private static List<DefaultNode> hilbertNodeSplit(final DefaultNode candidate) {
        ...
        if (isLeaf) {
            ...
            final DefaultNode resultA = (HilbertNode) ((splitListA.size() == 1) ? splitListA.get(0) : tree.createNode(tree, null, splitListA, null));
            final DefaultNode resultB = (HilbertNode) ((splitListB.size() == 1) ? splitListB.get(0) : tree.createNode(tree, null, splitListB, null));
    ...
    private static int findAnotherCell(int index, final DefaultNode candidate) {
        ...
        ArgumentChecks.ensureBetween("index to find another leaf is out of required limit",
                0, (int) Math.pow(2, ((Integer) ((HilbertNode) candidate).getUserProperty("hilbertOrder") * 2)), index);
        List<DefaultNode> listCells = (List<DefaultNode>) candidate.getUserProperty("cells");
    ...
    public DefaultNode createNode(Tree tree, DefaultNode parent, List<DefaultNode> listChildren, List<GeneralEnvelope> listEntries, double... coordinates) {
        ...
        return new HilbertNode(tree, parent, dp1, dp2, listChildren, listEntries);
    }
}
