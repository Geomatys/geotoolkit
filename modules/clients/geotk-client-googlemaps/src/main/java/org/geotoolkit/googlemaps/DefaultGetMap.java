...
public class DefaultGetMap extends AbstractRequest implements GetMapRequest{
    ...
    @Override
    public void setCenter(DirectPosition position) throws TransformException, FactoryException {
        
        //center must be expressed in long/lat
        final MathTransform trs = CRS.findMathTransform(
                position.getCoordinateReferenceSystem(), DefaultGeographicCRS.WGS84);
        position = trs.transform(position, position);
