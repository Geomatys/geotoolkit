...
final class ProjectionAnalyzer {
    ...
    private ProjectionAnalyzer(final ProjectedCRS crs) {
        ...
        if (candidate instanceof Parameterized) {
            group = ((Parameterized) candidate).getParameterValues();
        }
        if (group == null) {
            /*
    ...
    private ParameterDescriptorGroup getTransformDescriptor() {
        return (transform instanceof Parameterized) ?
            ((Parameterized) transform).getParameterDescriptors() : null;
    }
     ...
     * transform (it doesn't need to really exists) with {@code "scale factor"} == 1,
     * {@code "false easting"} == 0 and {@code "false northing"} == 0. In other words,
     * this method extracts the above-cited parameters in an affine transform.
     * <p>
