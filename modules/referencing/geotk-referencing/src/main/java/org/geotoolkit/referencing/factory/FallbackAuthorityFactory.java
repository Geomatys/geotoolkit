...
public class FallbackAuthorityFactory extends AuthorityFactoryAdapter {
    ...
    /**
     * Wraps the specified authority factories. If more than one element is specified,
     * then a chain of {@code FallbackAuthorityFactory} instances is created.
     *
    ...
    {
        return create(type, Arrays.asList(factories));
    }
            ...
            throws FactoryNotFoundException, ClassCastException
    {
        ...
        if (factories.isEmpty()) {
            throw new FactoryNotFoundException(Errors.format(Errors.Keys.FACTORY_NOT_FOUND_$1, type));
        }
            ...
            throws FactoryNotFoundException
    {
        ...
        if (factories.isEmpty()) {
            throw new FactoryNotFoundException(Errors.format(
                    Errors.Keys.FACTORY_NOT_FOUND_$1, AuthorityFactory.class));
        }
        ...
        if (factories.hasNext()) {
            ...
            while (fallback != primary) { // Paranoiac check
                if (!sameAuthorityCodes(fallback, primary)) {
                     ...
                     *
                     *   - A CachingAuthorityFactory wrapping the primary factory. Since
    ...
    final Collection<? super AuthorityFactory> dependencies() {
        final Collection<? super AuthorityFactory> dependencies = super.dependencies();
        dependencies.add(fallback);
        return dependencies;
    }
    ...
    /**
     * Invoked by <code>create<var>Foo</var>(String)</code> methods when the <cite>primary</cite>
     * factory failed to create an object. Note that it doesn't imply anything about the success
     * of <cite>fallback</cite> factory. The default implementation logs a message at the
    ...
    private static void notifyFailure(final String method, final FactoryException exception) {
        ...
        if (LOGGER.isLoggable(Level.FINE)) {
            final LogRecord record = Loggings.format(Level.FINE,
                    Loggings.Keys.FALLBACK_FACTORY_$1, exception);
            ...
            final AuthorityFactory primary, final AuthorityFactory fallback)
    {
        ...
        final Key key = new Key(mask, primary, fallback);
    ...
    /**
     * For keeping trace of {@code FallbackAuthorityFactory} instances created up to date.
     * The fields are the arguments given to the {@code create(mask, primary, fallback)} method.
     * The {@link #equals} method is implemented using identity comparisons because we don't
     * know how the user implemented its {@link AuthorityFactory#equals} method.
     */
    private static final class Key {
        /** Argument given to the {@code create} method. */ private final int mask;
        /** Argument given to the {@code create} method. */ private final AuthorityFactory primary;
        /** Argument given to the {@code create} method. */ private final AuthorityFactory fallback;

        Key(int mask, AuthorityFactory primary, AuthorityFactory fallback) {
            this.mask     = mask;
            this.primary  = primary;
            this.fallback = fallback;
        }

        /** Returns the hash code value for this key, using system hash code of factories. */
        @Override public int hashCode() {
            return mask + 31*(System.identityHashCode(primary) + 31*System.identityHashCode(fallback));
        }

        /** Compares this key with the given object, using identity comparisons of factories. */
        @Override public boolean equals(final Object other) {
            if (other instanceof Key) {
                final Key that = (Key) other;
                return mask == that.mask && primary == that.primary && fallback == that.fallback;
            }
            return false;
        }
    }
