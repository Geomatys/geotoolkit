        ...
        CSAuthorityFactory, DatumAuthorityFactory, CoordinateOperationAuthorityFactory
{
    ...
    /**
     * Returns the factories on which to delegate object creations. For any given
     * "{@code AUTHORITY:CODE}" string, this {@code MultiAuthoritiesFactory} will delegate the
     * object creation to the first factory from the list having an authority name equals to
     * the "{@code AUTHORITY}" part of the above string.
     * <p>
     * This factory list is determined from the collection given at construction time,
     ...
     */
    private static List<AuthorityFactory> createFallbacks(
            final Collection<? extends AuthorityFactory> factories)
    {
        ...
        return UnmodifiableArrayList.wrap(result.toArray(new AuthorityFactory[result.size()]));
    }
     ...
     * the {@linkplain DefaultNameSpace#DEFAULT_SEPARATOR default name separator} {@code ':'},
     * except if the code looks like a URL (e.g. {@code "http://www.opengis.net/"}), in which
     * case this method returns {@code '/'}.
     * <p>
     ...
     *
     * @param  code The code for which to returns the separator character.
     * @return The separator character for the given code.
     */
    protected char getSeparator(String code) {
        ...
        return DefaultNameSpace.DEFAULT_SEPARATOR;
    }
    ...
    static boolean isExcluded(final AuthorityFactory factory) {
        if (factory instanceof MultiAuthoritiesFactory) {
            return true;
        }
        if (factory instanceof AuthorityFactoryAdapter) {
            final AuthorityFactoryAdapter adapter = (AuthorityFactoryAdapter) factory;
            adapter.ensureInitialized(); // Must be invoked before adapter.getFactory.
            for (int f=0; f<TYPE_COUNT; f++) {
                if (isExcluded(adapter.getFactory(f))) {
                    return true;
                }
            }
        }
    ...
    /**
     * Searches for a factory of the given type. This method first searches in user-supplied
     * factories. If no user factory is found and this object is an instance of
     * {@link AllAuthoritiesFactory}, then this method request for a factory using
     * {@link AuthorityFactoryFinder}. The authority name is inferred from the specified code.
     *
     * @param  type The interface to be implemented.
    ...
    /**
     * Returns an arbitrary object authority factory for the specified {@code "AUTHORITY:NUMBER"}
     * code. This method delegates to <code>{@link #getAuthorityFactory(Class,String)
     * getAuthorityFactory}(AuthorityFactory.class, code)</code>.
     *
     ...
     */
    @Override
    protected final AuthorityFactory getAuthorityFactory(final String code)
            throws NoSuchAuthorityCodeException
    {
     ...
     * This method delegates to <code>{@link #getAuthorityFactory(Class,String)
     * getAuthorityFactory}(DatumAuthorityFactory.class, code)</code>.
     *
     * @param  code The code to parse.
     * @return The authority factory.
     * @throws NoSuchAuthorityCodeException if no authority name has been found.
     */
    @Override
    protected final DatumAuthorityFactory getDatumAuthorityFactory(final String code)
            throws NoSuchAuthorityCodeException
    {
     ...
     * This method delegates to <code>{@link #getAuthorityFactory(Class,String)
     * getAuthorityFactory}(CSAuthorityFactory.class, code)</code>.
     *
     * @param  code The code to parse.
     * @return The authority factory.
     * @throws NoSuchAuthorityCodeException if no authority name has been found.
     */
    @Override
    protected final CSAuthorityFactory getCSAuthorityFactory(final String code)
            throws NoSuchAuthorityCodeException
    {
     ...
     * This method delegates to <code>{@link #getAuthorityFactory(Class,String)
     * getAuthorityFactory}(CRSAuthorityFactory.class, code)</code>.
     *
     * @param  code The code to parse.
     * @return The authority factory.
     * @throws NoSuchAuthorityCodeException if no authority name has been found.
     */
    @Override
    protected final CRSAuthorityFactory getCRSAuthorityFactory(final String code)
            throws NoSuchAuthorityCodeException
    {
     ...
     * This method delegates to <code>{@link #getAuthorityFactory(Class,String)
     * getAuthorityFactory}(CoordinateOperationAuthorityFactory.class, code)</code>.
     *
     * @param  code The code to parse.
     * @return The authority factory.
     * @throws NoSuchAuthorityCodeException if no authority name has been found.
     */
    @Override
    protected final CoordinateOperationAuthorityFactory getCoordinateOperationAuthorityFactory(final String code)
            throws NoSuchAuthorityCodeException
    {
...
scanForType:    for (int i=0; i<FACTORY_TYPES.length; i++) {
     ...
     * @throws FactoryException if the query failed.
     */
    @Override
