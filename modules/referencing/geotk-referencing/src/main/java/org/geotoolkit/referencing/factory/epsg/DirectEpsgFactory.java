...
//TODO      case 9111: return NonSI.SEXAGESIMAL_DM;
    ...
    private synchronized Set<String> getAuthorityCodes0(final Class<?> type) throws FactoryException {
        ...
        for (int i=0; i<TABLES_INFO.length; i++) {
            ...
            if (table.type.isAssignableFrom(type) || type.isAssignableFrom(table.type)) {
                ...
                if (candidate == null) {
                    candidate = codes;
                    reference = new WeakReference<AuthorityCodes>(candidate);
            ...
            throws SQLException, FactoryException
    {
        ...
        stmt = prepareStatement(
                "Alias", "SELECT NAMING_SYSTEM_NAME, ALIAS" +
            ...
            throws NoSuchAuthorityCodeException, FactoryException
    {
        ...
        try {
            ...
            final PreparedStatement stmt;
            stmt = prepareStatement("PrimeMeridian",
    ...
    private AxisName getAxisName(final String code) throws FactoryException {
        ...
        if (returnValue == null) try {
            final PreparedStatement stmt;
            stmt = prepareStatement("AxisName",
            ...
            throws NoSuchAuthorityCodeException, FactoryException
    {
        ...
        try {
            ...
            stmt = prepareStatement("CoordinateSystem",
            ...
            while (result.next()) {
                ...
                if (cs == null) {
                    result.close();
                    throw new FactoryException(Errors.format(
                                               Errors.Keys.UNEXPECTED_DIMENSION_FOR_CS_$1, type));
                }
            ...
            throws NoSuchAuthorityCodeException, FactoryException
    {
        ...
        try {
            ...
            while (result.next()) {
                ...
                final ResultSet resultUnits = executeQuery(units, epsg);
                if (resultUnits.next()) {
            ...
            final ParameterValueGroup parameters) throws FactoryException, SQLException
    {
        ...
        final ResultSet result = executeQuery(stmt, method, operation);
        while (result.next()) {
            ...
            throws NoSuchAuthorityCodeException, FactoryException
    {
        ...
        try {
            ...
            final PreparedStatement stmt;
            stmt = prepareStatement("CoordinateOperation",
            ...
            while (result.next()) {
                ...
                if (isConversion && (sourceCRS==null || targetCRS==null)) {
                    ...
                    final ResultSet cr = executeQuery(cstmt, epsg);
            ...
            final String sourceCode, final String targetCode) throws FactoryException
    {
        ...
        try {
            ...
            do {
                ...
                final ResultSet result = executeQuery(stmt, sourceKey, targetKey);
                while (result.next()) {
