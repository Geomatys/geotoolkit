...
public class LineFormat extends Format {
    ...
    public LineFormat(final Format format) {
        if (format == null) {
            throw new NullArgumentException(Errors.format(Errors.Keys.NULL_FORMAT_$2, 0, 1));
        }
        ...
        for (int i=0; i<formats.length; i++) {
            if (formats[i] == null) {
                throw new NullArgumentException(Errors.format(
                        Errors.Keys.NULL_FORMAT_$2, i+1, formats.length));
            }
        }
    }
...
load:   while (true) {
            ...
            if (datum == null || next <= lower) {
                ...
                throw new ParseException(Errors.format(Errors.Keys.PARSE_EXCEPTION_$2,
    ...
    private Number getNumber(final int index) throws ParseException {
        ...
        try {
            ...
            ParseException exception = new ParseException(Errors.format(
                    Errors.Keys.UNPARSABLE_NUMBER_$1, data[index]), limits[index]);
    ...
    private void checkLength(final int expected) throws ParseException {
        if (count != expected) {
            ...
            throw new ParseException(Errors.format(count<expected ?
                                     Errors.Keys.LINE_TOO_SHORT_$2 : Errors.Keys.LINE_TOO_LONG_$3,
    ...
    private ParseException notAnInteger(final int i) {
        return new ParseException(Errors.format(Errors.Keys.NOT_AN_INTEGER_$1,
