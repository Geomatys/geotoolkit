...
public class AngleFormat extends Format {
    ...
    /**
     * Nombre minimal d'espaces que doivent occuper les parties entières des degrés (0),
     * minutes (1) et secondes (2). Le champs {@code widthDecimal} indique la largeur fixe
     * que doit avoir la partie décimale. Il s'appliquera au dernier champs non-zero dans
     * {@code width0..2}.
     */
    private int width0=1, width1=2, width2=0, widthDecimal=0;

    /**
     * Caractères à insérer au début ({@code prefix}) et à la suite des degrés, minutes et
     ...
     * la partie fractionnaire. La valeur {@code false} indique que les parties entières
     * et fractionnaires doivent être écrites ensembles (par exemple 34867 pour 34.867).
                ...
                if (upperCaseC == SYMBOLS[field]) {
                    ...
                    if (field != symbolIndex-1 || parseFinished) {
                        ...
                        throw new IllegalArgumentException(Errors.format(
                                Errors.Keys.ILLEGAL_ANGLE_PATTERN_$1, pattern));
                    }
                    if (c == upperCaseC) {
                        /*
    ...
    /**
     * Formats an angle. The string will be formatted according
     ...
     * @param pos
     *          An optional {@link FieldPosition} identifying a field in the formatted text, or
     *          {@code null} if this information is not wanted. This field position shall be
     *          constructed with one of the following constants: {@link #DEGREES_FIELD},
     ...
     * @param pos        Objet dans lequel mémoriser les index des premiers et derniers
     *                   caractères écrits, ou {@code null} pour ne pas mémoriser ces index.
     * @param w          Nombre de minimal caractères de la partie entière.
     * @param last       {@code true} si ce champs est le dernier, et qu'il faut donc écrire
     *                   la partie décimale.
     ...
     *
     * @param number
     *          Angle ou nombre à écrire.
     * @param type
     *          Type de l'angle ou du nombre: {@link #LONGITUDE}, {@link #LATITUDE} ou {@link #ALTITUDE}.
     * @param toAppendTo
     *          Buffer dans lequel écrire l'angle.
     * @param pos
     *          En entré, le code du champs dont on désire les index ({@link #DEGREES_FIELD},
     *          {@link #MINUTES_FIELD}, {@link #SECONDS_FIELD} ou {@link #HEMISPHERE_FIELD}).
     *          En sortie, les index du champs demandé. Ce paramètre peut être nul si cette
     *          information n'est pas désirée.
     *
     * @return Le buffer {@code toAppendTo} par commodité.
     */
    final StringBuffer format(final double number, final int type,
     ...
     *          information n'est pas désirée.
     * @param north
     *          Caractères à écrire si l'angle est positif ou nul.
     * @param south
    ...
    private StringBuffer format(final double angle, StringBuffer toAppendTo,
                                final FieldPosition pos, final char north, final char south)
    {
     ...
     *
     * @param source
     *          Chaîne dans laquelle on doit sauter le suffix.
     * @param pos
     *          En entré, l'index du premier caractère à considérer dans la chaîne {@code pos}.
     *          En sortie, l'index du premier caractère suivant le suffix (c'est-à-dire index
     *          à partir d'où continuer la lecture après l'appel de cette méthode). Si le suffix
     *          n'a pas été reconnu, alors cette méthode retourne par convention {@code SYMBOLS.length}.
     * @param field
     *          Champs à vérifier de préférences. Par exemple la valeur 1 signifie que les suffix
     *          des minutes et des secondes devront être vérifiés avant celui des degrés.
     ...
     * chaîne "{@code 48°12.34'}" même si le patron attendu était "{@code DDMM.mm}" (c'est-à-dire
     * que la chaîne aurait du être "4812.34"). Les espaces entre les degrés, minutes et secondes
     * sont acceptés. Si l'angle est suivit d'un symbole "N" ou "S", alors l'objet retourné sera
     * de la classe {@link Latitude}. S'il est plutot suivit d'un symbole "E" ou "W", alors l'objet
     * retourné sera de la classe {@link Longitude}. Sinon, il sera de la classe {@link Angle}.
     *
