 ...
 *
 *    (C) 1998-2010, Open Source Geospatial Foundation (OSGeo)
...
public class Line implements Cloneable, Serializable {
     ...
     */
    public double fit(final double[] x, final double[] y) throws MismatchedSizeException {
        final int N = x.length;
        if (N != y.length) {
            throw new MismatchedSizeException();
        }
        int    count  = 0;
        double mean_x = 0, rx=0;
        double mean_y = 0, ry=0;
        for (int i=0; i<N; i++) {
            double xi = x[i];
            double yi = y[i];
            if (!isNaN(xi) && !isNaN(yi)) {
                // Kahan summation algorithm.
                xi += rx; rx = xi + (mean_x - (mean_x += xi));
                yi += ry; ry = yi + (mean_y - (mean_y += yi));
                count++;
            }
        }
        mean_x /= count;
         ...
         */
        double mean_x2 = 0; rx=0;
        double mean_y2 = 0; ry=0;
        double mean_xy = 0, rc=0;
        for (int i=0; i<N; i++) {
            double xi = x[i];
            double yi = y[i];
            if (!isNaN(xi) && !isNaN(yi)) {
                ...
                double xy = xi*yi;
                // Kahan summation algorithm.
                xx += rx; rx = xx + (mean_x2 - (mean_x2 += xx));
                yy += ry; ry = yy + (mean_y2 - (mean_y2 += yy));
                xy += rc; rc = xy + (mean_xy - (mean_xy += xy));
            }
        }
        mean_x2 /= count;
        mean_y2 /= count;
         ...
         */
        final double slope = mean_xy / mean_x2;
        setLine(slope, mean_y - mean_x*slope);
    ...
    public boolean equals(final Object object) {
        if (object!=null && getClass().equals(object.getClass())) {
            ...
            return Utilities.equals(this.slope, that.slope) &&
                   ...
                   Utilities.equals(this.x0,    that.x0   );
        } else {
            return false;
        }
    }
    ...
    @Override
    public int hashCode() {
        final long code = doubleToLongBits(slope) + 31*doubleToLongBits(y0);
