 ...
 * block of a {@code try} block begining immediatly after the call to {@link #lock lock},
        ...
        if (value instanceof Handler<?>) {
            ...
            if (work.isHeldByCurrentThread()) {
                if (isKeyCollisionAllowed()) {
                     ...
                     * CoordinateOperation, which is associated to the first occurence of that key.
                     */
