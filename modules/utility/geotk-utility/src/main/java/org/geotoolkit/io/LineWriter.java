...
public class LineWriter extends FilterWriter {
     ...
     * @throws NullArgumentException if {@code out} or {@code lineSeparator} is {@code null}.
     */
     ...
     * @throws NullArgumentException If {@code lineSeparator} is {@code null}.
     */
     ...
     * <strong>not</strong> contains any line separator.
     */
    private void writeLine(final String string, final int lower, int upper) throws IOException {
        while (upper != lower) {
            final char c = string.charAt(upper-1);
            assert (c!='\r' && c!='\n');
            if (isWhitespace(c)) {
                upper--;
                continue;
            ...
            flushBuffer();
            out.write(string, lower, upper-lower);
            return;
        }
        assert bufferBlank();
        count = 0;
                ...
                default: {
                    if (c >= Character.MIN_VALUE && c <= Character.MAX_VALUE && isWhitespace((char)c)) {
                        if (count >= buffer.length) {
                            buffer = Arrays.copyOf(buffer, count + Math.min(8192, count));
                        }
                        buffer[count++] = (char)c;
                    } else {
                        flushBuffer();
